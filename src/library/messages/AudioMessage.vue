<template>
  ...
  >
    ...
    <p v-if="message.subText" class="audio-message__subtext">
      {{ message.subText }}
    </p>
    ...
    <div class="audio-message__content" @mouseenter="showMenu">
      ...
      <div class="audio-message__audio-container">
        <audio ref="player" :src="message.url" type="audio/webm" />
        <button v-show="!isPlaying" class="audio-message__play" @click="togglePlayPause">
          <span class="pi pi-play" />
        </button>
        <button v-show="isPlaying" class="audio-message__pause" @click="togglePlayPause">
      ...
      <div v-if="message.transcript?.text" class="audio-message__transcript-container">
        <p @click="isFullTranscript = !isFullTranscript">
          {{ message.transcript.text }}
        </p>
      </div>
      <div v-if="message.text" class="audio-message__text-container">
        <p @click="inNewWindow" v-html="linkedText" />
      </div>
      ...
      <EmbedPreview v-if="message.embed" :class="message.position" :embed="message.embed" />

      <div class="audio-message__info-container">
        <div v-if="message.views" class="audio-message__views" @click="viewsAction">
        ...
        <div
          ...
          :class="status"
        >
          <span v-if="message.status !== 'sent'" class="pi pi-check" />
        ...
        <transition name="modal-fade">
          ...
          >
            <div class="audio-message__modal">
              <button class="audio-message__modal-close-button" @click="isFullTranscript = false">
                <span>
                  <i class="pi pi-times" />
                </span>
              </button>
              <p style="word-wrap: break-word; max-width: 25rem">
                {{ message.transcript?.text }}
              </p>
            </div>
          </div>
  ...
  watch(
    ...
    () => {
      if (player.value) {
        ...
        if (currentTime.value == audioDuration.value) isPlaying.value = false
      }
    }
  )
