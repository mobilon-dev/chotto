...
function handleKey(event) {
  if (
    event.target ==
    document.getElementById('placeholder' + chatAppId + props.index)
  ) {
    if (event.key === 'Escape') {
      event.target.blur()
    }
    if (event.key == 'Tab') {
      event.target.blur()
      if (
        document.getElementById('placeholder' + chatAppId + (props.index + 1))
      ) {
        event.preventDefault()
        document
          .getElementById('placeholder' + chatAppId + (props.index + 1))
          .focus()
      }
    }
  }
