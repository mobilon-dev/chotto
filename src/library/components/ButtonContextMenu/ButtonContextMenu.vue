  ...
  if (actionScope.value && contextMenu.value) {
    ...
    nextTick(() => {
      const r = {
        left: {
          top:
            bounds.top - (cmBounds.height - bounds.height) / 2 - cmBounds.top,
          left: bounds.left - cmBounds.width - cmBounds.left,
        },
        right: {
          top:
            bounds.top - (cmBounds.height - bounds.height) / 2 - cmBounds.top,
          left: bounds.left + bounds.width - cmBounds.left,
        },
        bottom: {
          top: bounds?.bottom - cmBounds.top,
          left:
            bounds?.left - (cmBounds.width - bounds.width) / 2 - cmBounds.left,
        },
        top: {
          top: bounds.top - cmBounds.height - cmBounds.top,
          left:
            bounds?.left - (cmBounds.width - bounds.width) / 2 - cmBounds.left,
        },
      }
...
const handleClickOutside = (event) => {
  if (
    props.mode == 'click' &&
    actionScope.value &&
    !actionScope.value.contains(event.target)
  ) {
    hideMenu()
  }
}
...
onMounted(() => {
  nextTick(() => {
    contextMenu.value = document.getElementById(
      'context-menu-' + buttonContextMenuId + extChatAppId
    )
    if (contextMenu.value) {
      hideMenu()
