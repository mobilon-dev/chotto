  ...
  <div>
    ...
    >
      <div class="chat-item__avatar-container">
        <span class="chat-item__status-user" :style="{ backgroundColor: props.chat.status }" />

        <UserAvatar :avatar="props.chat.avatar" />
      </div>
      ...
      <div class="chat-item__info-container">
        ...
        <Tooltip :text="chat.lastMessage" position="bottom">
          <div v-if="chat.lastMessage || chat.typing" class="chat-item__last-message">
            {{ showText }}
          </div>
        </Tooltip>
      </div>
      ...
      <div class="chat-item__details-container">
        <div v-if="chat['lastActivity.time'] && !buttonMenuVisible" class="chat-item__time">
        ...
        <div
          ...
          :style="{
            backgroundColor: chat.colorUnread ? chat.colorUnread : null,
          }"
        >
        ...
        <div v-if="chat.countUnread < 1" class="chat-item__status-chat-container">
          <div
            v-if="statuses.includes(chat['lastMessage.status'])"
            class="chat-item__status-message"
            :class="status"
          >
            <span v-if="chat['lastMessage.status'] !== 'sent'" class="pi pi-check" />
      ...
      >
        <button v-if="!chat.dialogsExpanded" id="noSelectButton" class="chat-item__menu-button">
    ...
    <div v-if="chat.dialogsExpanded" class="dialog__container">
      <div
      ...
      >
        ...
        <div
          ...
          class="chat-item__unread"
          :style="{
            backgroundColor: dialog.colorUnread ? dialog.colorUnread : null,
          }"
        >
...
<script setup lang="ts">
  ...
  const getSortedDialogs = () => {
    return props.chat.dialogs.toSorted((a, b) => {
  ...
  const onMouseLeave = (event) => {
    if (event.relatedTarget?.className == 'context-menu__list') buttonMenuVisible.value = true
