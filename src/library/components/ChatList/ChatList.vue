<template>
  <div class="chat-list">
    <div class="chat-list__title-container">
      <h2 v-if="titleEnabled" class="chat-list__title">
    ...
    <ChatFilter v-if="filterEnabled" class="chat-list__filter" @update="getFilter" />
    ...
    >
      ...
      <div class="chat-list__scrollable-items">
        <ChatItem
          v-for="chat in getSortedAndFilteredChats().filter(
            (c) => !c.isFixedBottom && !c.isFixedTop
          )"
          :key="chat.chatId"
          class="chat-list__item"
          :chat="chat"
    ...
    <transition>
      <button v-if="isShowButton" class="chat-list__button-up" @click="scrollToTopForce">
...
<script setup>
  ...
  watch(
    () => allowLoadMore.value,
    () => {
      if (allowLoadMore.value) emit('loadMoreChats')
    }
  )
  ...
  const selectDialog = (dialog) => {
    props.chats.forEach((c) => {
      ...
      if (c.dialogs)
        c.dialogs.forEach((d) => {
          d.isSelected = false
          if (d.dialogId == dialog.dialogId) d.isSelected = true
        })
    })
  }
    ...
    return props.chats
      ...
      .filter((c) => {
        if (!props.filterQuery)
          return c.name.includes(filter.value) || c.metadata.includes(filter.value)
        else return c.name.includes(props.filterQuery) || c.metadata.includes(props.filterQuery)
      })
  }
