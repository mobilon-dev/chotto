  ...
  <div class="audio-recorder__container">
    ...
    <button
      v-if="
        ...
        !getMessage().isRecording
      "
      class="audio-recorder__button"
      :class="{
        'audio-recorder__button-disabled':
          state == 'disabled' || getMessage().file,
      }"
...
const { getMessage, setMessageFile, resetMessageFile, setRecordingMessage } =
...
const stopAudioRecording = () => {
  if (mediaRecorder.value) {
    ...
    mediaRecorder.value.onstop = async () => {
      const file = new File(chunks.value, 'voicemessage.mp3', {
        type: 'audio/*',
      })
      const url = URL.createObjectURL(file)
      audio.value = url
      uploadStatus.value = 'uploading'
      await uploadFile(props.filebumpUrl, file).then((data) => {
        uploadStatus.value = data.status
        if (data.status == 'success') {
          const previewContainer = document.getElementById(
            'chat-input-file-line-' + chatAppId
          )
          if (previewContainer) {
