<template>
  ...
  >
    ...
    <transition>
      ...
      >
        <div v-if="buttonParams" class="message-feed__unread-amount">
  ...
  <div v-else ref="refFeed" class="message-feed">
...
const showKeyboard = computed(() => {
  if (
    isKeyboardPlace.value &&
    props.objects.length > 0 &&
    props.objects[props.objects.length - 1].keyboard
  )
    return true;
  else return false;
});
...
watch(
  ...
  () => {
    ...
    if (props.scrollToBottom) performScrollToBottom();
  },
  { immediate: true }
);
...
const feedObjectDoubleClick = (event: MouseEvent, object: IFeedObject) => {
  if (props.enableDoubleClickReply) {
    ...
    if (
      object.type.indexOf("system") == -1 &&
      object.type.indexOf("typing") == -1
    ) {
      const previewContainer = document.getElementById(
        "chat-input-reply-line-" + chatAppId
      );
      if (previewContainer) {
...
watch(
  ...
  () => {
    nextTick(() => {
      ...
      if (isScrollByMouseButton.value) {
        const element = unref(refFeed);
        if (movingDown.value) element.scrollTop = element.scrollHeight - 400;
        if (!movingDown.value) element.scrollTop = 400;
      }
  ...
  () => {
    if (props.scrollTo) {
      ...
      if (target instanceof HTMLElement && list instanceof HTMLElement)
        list.scrollTop =
          target.offsetTop + target.clientHeight / 2 - list.clientHeight / 2;
      document
        .getElementById(elem)
        ?.children[0].classList.add("focused-message");
      setTimeout(() => {
        document
          .getElementById(elem)
          ?.children[0].classList.remove("focused-message");
      }, 2000);
    }
  }
);
...
.focused-message {
  background-color: color-mix(
    in srgb,
    var(--chotto-message-focused-color) 30%,
    transparent
  );
  box-shadow: 0px 0px 12px 2px
    color-mix(in srgb, var(--chotto-message-focused-color) 30%, transparent);
}
