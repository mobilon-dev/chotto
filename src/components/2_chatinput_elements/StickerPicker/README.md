...
- Определяет тип файла по расширению URL (webp, tgs)
- **Поддерживает анимированные стикеры TGS** с оптимизацией производительности
- **Управление анимацией при наведении** - анимация запускается только при наведении курсора
...
### Оптимизация производительности анимированных стикеров

Для снижения нагрузки на браузер и улучшения производительности:
- Библиотеки TGS (~700KB) загружаются **только при наличии** TGS стикеров в наборе
- Анимация в сетке стикеров и иконках вкладок **не запускается автоматически**
- Анимация запускается **только при наведении курсора** на стикер или вкладку
- При уходе курсора анимация автоматически останавливается
- При закрытии пикера все анимации останавливаются
- В предпросмотре анимация запускается автоматически для лучшего UX
...
[
  {
    id: 'set1', // опционально, по умолчанию генерируется как 'tab-0', 'tab-1', и т.д.
    label: 'Набор 1', // опционально, по умолчанию 'Set 1', 'Set 2', и т.д.
    iconUrl: 'https://example.com/icon1.webp', // опционально, по умолчанию первый стикер из массива stickers
    stickers: [ // обязательно
      { url: 'https://example.com/sticker1.webp', alt: 'Sticker 1' },
      { url: 'https://example.com/sticker2.webp', alt: 'Sticker 2' },
    ]
  },
...
**Примечания:**
- Поле `stickers` является обязательным для каждого объекта
- Поля `id`, `label` и `iconUrl` опциональны и имеют значения по умолчанию
- Если `iconUrl` не указан, используется URL первого стикера из массива `stickers`
- Если `label` не указан, генерируется как `Set 1`, `Set 2`, и т.д.
- Если `id` не указан, генерируется как `tab-0`, `tab-1`, и т.д.
...
- `.tgs` - анимированные стикеры Telegram (type: 'sticker')

### Поддержка анимированных стикеров (TGS)

Компонент поддерживает анимированные стикеры формата TGS (Telegram Animated Stickers):
- **Автоматическое определение** - компонент автоматически определяет TGS файлы по расширению `.tgs` в URL
- **Динамическая загрузка библиотек** - библиотеки для воспроизведения TGS (~700KB) загружаются только при наличии TGS стикеров в наборе
- **Оптимизация производительности** - анимация не запускается автоматически, что снижает нагрузку на браузер

#### Управление анимацией при наведении

Для оптимизации производительности анимированные стикеры ведут себя следующим образом:

**В сетке стикеров:**
- Анимация **не запускается автоматически** при загрузке
- Анимация **запускается** при наведении курсора на стикер (`mouseenter`)
- Анимация **останавливается** при уходе курсора (`mouseleave`)
- Анимация **останавливается** при закрытии пикера

**В иконках вкладок:**
- Анимация **не запускается автоматически** при загрузке
- Анимация **запускается** при наведении курсора на вкладку (`mouseenter`)
- Анимация **останавливается** при уходе курсора (`mouseleave`)

**В предпросмотре:**
- Анимация **запускается автоматически** при показе предпросмотра (для лучшего UX)
